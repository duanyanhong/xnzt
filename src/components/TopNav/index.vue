  ...
  computed: {
    ...
    activeMenu() {
      ...
      if (path !== undefined && path.lastIndexOf("/") > 0 && hideList.indexOf(path) === -1) {
        ...
        activePath = path;
        if(activePath=='/index'){
          activePath=sessionStorage.getItem('defaultMenu')
        }
        this.$store.dispatch('app/toggleSideBarHide', false);
      }
              ...
              if (route.children[i].hidden == false) {
                ...
                if (firstUnHiddenChild.children != null) {
                  for (let j = 0; j < firstUnHiddenChild.children.length; j++) {
                    ...
                    if (firstUnHiddenChild.children[j].hidden == false) {
                      ...
                      let completePath = path.resolve(firstUnHiddenChild.path,firstUnHiddenChildTmp.path)
                      this.$router.push({
                        path: completePath,
                        query: firstUnHiddenChildTmp.query == undefined ? null : JSON.parse(firstUnHiddenChildTmp.query),
                      });
                      break;
                    }
                  }
                ...
                //该路由不包含子路由，即只有两级菜单，直接跳转
                  this.$router.push({
                    ...
                    query:  firstUnHiddenChild.query == undefined ? null : JSON.parse(firstUnHiddenChild.query),
                  });
                }
                break;
              }
