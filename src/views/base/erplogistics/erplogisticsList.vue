  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col>
        ...
        <div class="crud-opts">
          <span class="crud-opts-left">
            ...
            <el-button
              ...
              @click="addShow"
              plain
              v-hasPermi="['Base:Erplogistics:Add']"
            >
              新增
            </el-button>
            <!--右侧-->
          ...
          <span class="crud-opts-left">
            ...
            <el-upload
              ...
              accept=".xlsx,.xls,.csv"
              action="#"
              v-hasPermi="['Base:Erplogistics:Import']"
            >
          ...
          <span class="crud-opts-left">
            <el-button
              ...
              plain
              @click="downMyOrder"
              v-hasPermi="['Base:Erplogistics:Export']"
            >导出</el-button>
          </span>
        </div>
        <u-table
        ...
        >
          ...
          <u-table-column :show-overflow-tooltip="true" prop="erplogisticsCode" label="erp物流编码" />
          <u-table-column :show-overflow-tooltip="true" prop="erplogisticsName" label="erp物流名称" />
          ...
          >
            <template slot-scope="scope">
              <el-button v-hasPermi="['Base:Erplogistics:Edit']" size="mini" type="text" style="width: 50px;" icon="el-icon-edit" @click="editStaff(scope.row)">修改</el-button>
              <el-popover v-hasPermi="['Base:Erplogistics:Disable']" v-if="scope.row.disable=='否'" :ref="`popover-${scope.$index}`" placement="top" width="200">
              ...
              <el-popover v-hasPermi="['Base:Erplogistics:Disable']" v-if="scope.row.disable=='是'" :ref="`popover-${scope.$index}`" placement="top" width="200">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :visible.sync="formIsshow" :title="titleContent" width="500px">
      <el-form ref="formData" :inline="true" :model="formData" :rules="rules" size="small" label-width="150px">
        ...
        <el-form-item label="erp物流编码" prop="erplogisticsCode">
          <el-input v-model="formData.erplogisticsCode" style="width: 250px" placeholder="请输入erp物流编码" />
        </el-form-item>
        <el-form-item label="erp物流名称" prop="erplogisticsName">
          <el-input v-model="formData.erplogisticsName" style="width: 250px" placeholder="请输入erp物流名称" />
        </el-form-item>
      </el-form>
...
<script>
  import { getErplogisticsList,editErplogistics,addErplogistics,disableErplogistics,importErplogistics,exportErplogistics } from "@/api/base/erplogistics";
  export default {
    name: 'Erplogistics',
    data() {
      return {
        rules: {
          erplogisticsCode: [
            { required: true, message: '请输入erp物流编码', trigger: 'blur' }
          ],
          erplogisticsName: [
            { required: true, message: '请输入erp物流名称', trigger: 'blur' }
          ],
          ownerId: [
  ...
  mounted() {
    ...
    methods: {
      ...
      loadData() {
        this.loading = true
        getErplogisticsList(this.searchData).then(res => {
          if(res){
            this.loading = false
      ...
      addForm() {
        this.$refs['formData'].validate((valid) => {
          if (valid) {
            var _this = this
            if (_this.editStatus) {
              editErplogistics(_this.formData).then(res => {
                if(res){
              ...
              addErplogistics(_this.formData).then(res => {
                if(res){
                  _this.formIsshow = false
                  _this.$modal.notifySuccess(res.errMsg)
                  _this.loadData()
      ...
      uploadSuccess(file){
        ...
        importErplogistics(param).then(res => {
          this.loading = false
          if(res){
      ...
      downMyOrder() {
        this.loading = true
        exportErplogistics(this.searchData).then(res => {
          this.loading = false
          if(res){
            this.$modal.notifySuccess(res.errMsg)
            if (res.msgData.downloadUrl !== undefined && res.msgData.downloadUrl !== '') {
      ...
      editDisable(scope) {
        ...
        disableErplogistics(data).then(res => {
          if(res){
            this.$modal.notifySuccess(res.errMsg)
            this.loadData()
          }
