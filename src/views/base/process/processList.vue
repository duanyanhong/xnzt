  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col>
        ...
        >
          <u-table-column :show-overflow-tooltip="true" prop="processCode" label="流程编码" />
          <u-table-column :show-overflow-tooltip="true" prop="processName" label="流程名称" />
          ...
          >
            <template slot-scope="scope">
              <el-button v-hasPermi="['Base:Process:Edit']" size="mini" type="text" style="width: 50px;" icon="el-icon-edit" @click="editStaff(scope.row)">修改</el-button>
              <el-popover v-hasPermi="['Base:Process:Disable']" v-if="scope.row.disable=='否'" :ref="`popover-${scope.$index}`" placement="top" width="200">
              ...
              <el-popover v-hasPermi="['Base:Process:Disable']" v-if="scope.row.disable=='是'" :ref="`popover-${scope.$index}`" placement="top" width="200">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :visible.sync="formIsshow" :title="titleContent" width="500px">
      <el-form ref="formData" :inline="true" :model="formData" :rules="rules" size="small" label-width="120px">
        <el-form-item label="流程编码" prop="processCode">
          <el-input v-model="formData.processCode" style="width: 250px" placeholder="请输入流程编码" />
        </el-form-item>
        <el-form-item label="流程名称" prop="processName">
          <el-input v-model="formData.processName" style="width: 250px" placeholder="请输入流程名称" />
        </el-form-item>
      </el-form>
...
<script>
import { getProcessList,editProcess,addProcess,disableProcess } from "@/api/base/process";
export default {
  name: 'Process',
  data() {
    return {
  ...
  methods: {
    ...
    loadData() {
      this.loading = true
      getProcessList(this.searchData).then(res => {
        if(res){
          this.loading = false
    ...
    addForm() {
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          var _this = this
          if (_this.editStatus) {
            editProcess(_this.formData).then(res => {
              if(res){
            ...
            addProcess(_this.formData).then(res => {
              if(res){
                _this.formIsshow = false
                _this.$modal.notifySuccess(res.errMsg)
                _this.loadData()
