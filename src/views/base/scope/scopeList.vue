  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col>
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-button v-hasPermi="['Base:Scope:Detail']" size="mini" type="text"  icon="el-icon-link" @click="shineDetail(scope.row)">映射详情</el-button>
              <el-popover v-hasPermi="['Base:Scope:Delete']"  :ref="`popovers-${scope.$index}`" placement="top" width="200">
              ...
              <el-popover v-hasPermi="['Base:Scope:Disable']" v-if="scope.row.disable=='否'" :ref="`popover-${scope.$index}`" placement="top" width="200">
              ...
              <el-popover v-hasPermi="['Base:Scope:Disable']" v-if="scope.row.disable=='是'" :ref="`popover-${scope.$index}`" placement="top" width="200">
    ...
    <el-dialog title="映射详情" :visible.sync="dialogTableVisible" width="500px">
      <el-table :data="gridData" :row-height="50" :height="350" >
        <el-table-column property="platformCode" align="center"  label="平台编码"></el-table-column>
        <el-table-column property="platformName" align="center"  label="平台名称"></el-table-column>
...
<script>
import { getScopeList,deleteScope,addScope,disableScope,importScope,exportScope,getProvince,getCity,platformscopeDetail } from "@/api/base/scope";
export default {
  name: 'Scope',
  data() {
    return {
  ...
  methods: {
    ...
    loadData() {
      this.loading = true
      getScopeList(this.searchData).then(res => {
        if(res){
          this.loading = false
    ...
    addForm() {
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          var _this = this
          addScope(_this.formData).then(res => {
            if(res){
    ...
    downMyOrder() {
      this.loading = true
      exportScope(this.searchData).then(res => {
        this.loading = false
        if(res){
          this.$modal.notifySuccess(res.errMsg)
          if (res.msgData.downloadUrl !== undefined && res.msgData.downloadUrl !== '') {
    ...
    getProvince(){
      ...
      this.city = []
      if(this.formData.scopeType=='省份' || this.formData.scopeType==''){
        this.province = []
        return;
      }
    ...
    shineDetail(row){
      const data = {accessToken:sessionStorage.getItem('xn_token'),id:row.id}
      this.gridData=[]
      platformscopeDetail(data).then(res => {
        if(res){
          if(res.msgData.details.length<1){
            this.$modal.notifyWarning('暂无映射地址！')
            return
          }
