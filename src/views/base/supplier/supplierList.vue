    ...
    <el-row :gutter="20">
      ...
      <el-col>
        ...
        >
          ...
          <u-table-column :show-overflow-tooltip="true" prop="supplierName" label="供应商名称" />
          ...
          <u-table-column :show-overflow-tooltip="true" prop="contactEmail" label="邮箱地址" />
          <u-table-column :show-overflow-tooltip="true" prop="country" label="国家" />
          ...
          >
            <template slot-scope="scope">
              <el-button v-hasPermi="['Base:Supplier:Edit']" size="mini" type="text" style="width: 50px;" icon="el-icon-edit" @click="editStaff(scope.row)">修改</el-button>
              <el-popover v-hasPermi="['Base:Supplier:Disable']" v-if="scope.row.disable=='否'" :ref="`popover-${scope.$index}`" placement="top" width="200">
              ...
              <el-popover v-hasPermi="['Base:Supplier:Disable']" v-if="scope.row.disable=='是'" :ref="`popover-${scope.$index}`" placement="top" width="200">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :visible.sync="formIsshow" :title="titleContent" width="720px">
      <el-form ref="formData" :inline="true" :model="formData" :rules="rules" size="small" label-width="120px">
        ...
        <el-form-item label="供应商名称" prop="supplierName">
          <el-input v-model="formData.supplierName" style="width: 200px" placeholder="请输入供应商名称" />
        </el-form-item>
        ...
        <el-form-item label="邮箱地址" prop="contactEmail">
          <el-input v-model="formData.contactEmail" style="width: 200px" placeholder="请输入邮箱地址" />
        </el-form-item>
        <el-form-item label="国家" prop="country">
          <el-input v-model="formData.country" style="width: 200px" placeholder="请输入国家" />
        </el-form-item>
...
<script>
import { getSupplierList,editSupplier,addSupplier,disableSupplier } from "@/api/base/supplier";
export default {
  name: 'Supplier',
  data() {
    return {
  ...
  methods: {
    ...
    loadData() {
      this.loading = true
      getSupplierList(this.searchData).then(res => {
        if(res){
          this.loading = false
    ...
    addForm() {
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          var _this = this
          if (_this.editStatus) {
            editSupplier(_this.formData).then(res => {
              if(res){
            ...
            addSupplier(_this.formData).then(res => {
              if(res){
                _this.formIsshow = false
                _this.$modal.notifySuccess(res.errMsg)
                _this.loadData()
