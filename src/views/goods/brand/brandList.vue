  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col>
        ...
        >
          <u-table-column :show-overflow-tooltip="true" prop="brandCode" label="品牌编码" />
          <u-table-column :show-overflow-tooltip="true" prop="brandName" label="品牌名称" />
          ...
          >
            <template slot-scope="scope">
              <el-button v-hasPermi="['Goods:Goodsbrand:Edit']" size="mini" type="text" style="width: 50px;" icon="el-icon-edit" @click="editStaff(scope.row)">修改</el-button>
              <el-popover v-hasPermi="['Goods:Brand:Edit']" v-if="scope.row.disable=='否'" :ref="`popover-${scope.$index}`" placement="top" width="200">
              ...
              <el-popover v-hasPermi="['Goods:Brand:Disable']" v-if="scope.row.disable=='是'" :ref="`popover-${scope.$index}`" placement="top" width="200">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :visible.sync="formIsshow" :title="titleContent" width="500px">
      <el-form ref="formData" :inline="true" :model="formData" :rules="rules" size="small" label-width="120px">
        <el-form-item label="品牌编码" prop="brandCode">
          <el-input v-model="formData.brandCode" style="width: 250px" placeholder="请输入品牌编码" />
        </el-form-item>
        <el-form-item label="品牌名称" prop="brandName">
          <el-input v-model="formData.brandName" style="width: 250px" placeholder="请输入品牌名称" />
        </el-form-item>
      </el-form>
...
<script>
  import { getGoodsbrandList,editGoodsbrand,addGoodsbrand,disableGoodsbrand } from "@/api/goods/goodsbrand";
  export default {
    name: 'Goodsbrand',
    data() {
      return {
    ...
    methods: {
      ...
      loadData() {
        this.loading = true
        getGoodsbrandList(this.searchData).then(res => {
          if(res){
            this.loading = false
      ...
      addForm() {
        this.$refs['formData'].validate((valid) => {
          if (valid) {
            var _this = this
            if (_this.editStatus) {
              editGoodsbrand(_this.formData).then(res => {
                if(res){
              ...
              addGoodsbrand(_this.formData).then(res => {
                if(res){
                  _this.formIsshow = false
                  _this.$modal.notifySuccess(res.errMsg)
                  _this.loadData()
      ...
      editDisable(scope) {
        ...
        disableGoodsbrand(data).then(res => {
          if(res){
