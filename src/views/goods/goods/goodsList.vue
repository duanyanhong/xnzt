    ...
    <el-dialog append-to-body :close-on-click-modal="false" :visible.sync="formShopIsshow" title="转门店库存" width="500px">
      <el-form :inline="true"  size="small" label-width="120px">
        <el-form-item label="逻辑仓" prop="logicwarehouseId">
          ...
          >
            <el-option
              v-for="item in vlogicwarehouses"
              :key="item.logicwarehouseId"
              :label="item.logicwarehouseName"
              :value="item.logicwarehouseId"
            />
...
export default {
  ...
  data() {
    return {
      ...
      siteId:[],
      logicwarehouseId: [],
      uploadRequest:'/xnkj/goods/goods/uploadImg', //上传图片
    }
  },
  created() {
  ...
  methods: {
    ...
    loadData() {
      ...
      getGoodsList(this.searchData).then(res => {
        if(res){
          ...
          this.logicwarehouses = res.msgData.slogicwarehouses
          this.carriers = res.msgData.carriers
          this.vlogicwarehouses = res.msgData.vlogicwarehouses
        }
      })
    },
    // 搜索
    ...
    getSite(){
      this.siteId = []
      this.sites = []
      const data = {logicwarehouseId: this.logicwarehouseId, accessToken: sessionStorage.getItem('xn_token')}
