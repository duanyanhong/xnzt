  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col>
        ...
        >
          <u-table-column :show-overflow-tooltip="true" prop="markName" label="标记内容" />
          <u-table-column label="标记示例"  align="center">
            <template slot-scope="scope">
              <el-tag  :color="scope.row.markColor" ><span style="color:#fff;">{{ scope.row.markName }}</span></el-tag>
            </template>
          </u-table-column>
          ...
          >
            <template slot-scope="scope">
              <el-button v-hasPermi="['Goods:Mark:Edit']" size="mini" type="text" style="width: 50px;" icon="el-icon-edit" @click="editStaff(scope.row)">修改</el-button>
              <el-popover v-hasPermi="['Goods:Mark:Disable']" v-if="scope.row.disable=='否'" :ref="`popover-${scope.$index}`" placement="top" width="200">
              ...
              <el-popover v-hasPermi="['Goods:Mark:Disable']" v-if="scope.row.disable=='是'" :ref="`popover-${scope.$index}`" placement="top" width="200">
...
<script>
  import { getGoodsmarkList,editGoodsmark,addGoodsmark,disableGoodsmark } from "@/api/goods/goodsmark";
  export default {
    name: 'Goodsmark',
    data() {
      return {
    ...
    methods: {
      ...
      loadData() {
        this.loading = true
        getGoodsmarkList(this.searchData).then(res => {
          if(res){
            this.loading = false
