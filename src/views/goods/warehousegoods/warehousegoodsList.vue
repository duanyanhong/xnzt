...
<!--              @click="addShow"-->
<!--              plain-->
<!--              v-hasPermi="['Goods:WarehouseGoods:Add']"-->
<!--            >-->
<!--              新增-->
<!--            </el-button>-->
            <el-button
              ...
              plain
              v-hasPermi="['Goods:WarehouseGoods:Synch']"
            >
              同步
            </el-button>
            <!--右侧-->
          ...
          <span class="crud-opts-left">
            ...
            <el-upload
              ...
              accept=".xlsx,.xls,.csv"
              action="#"
              v-hasPermi="['Goods:WarehouseGoods:Import']"
            >
          ...
          <span class="crud-opts-left">
            <el-button
              ...
              plain
              @click="downMyOrder"
              v-hasPermi="['Goods:WarehouseGoods:Export']"
            >导出</el-button>
          </span>
        </div>
        <u-table
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-button v-hasPermi="['Goods:WarehouseGoods:Edit']" size="mini" type="text" style="width: 50px;" icon="el-icon-edit" @click="editStaff(scope.row)">修改</el-button>
              <el-popover v-hasPermi="['Goods:WarehouseGoods:Disable']" v-if="scope.row.disable=='否'" :ref="`popover-${scope.$index}`" placement="top" width="200">
              ...
              <el-popover v-hasPermi="['Goods:WarehouseGoods:Disable']" v-if="scope.row.disable=='是'" :ref="`popover-${scope.$index}`" placement="top" width="200">
...
<script>
  import { getWarehousegoodsList,editWarehousegoods,addWarehousegoods,disableWarehousegoods,getLogicwarehouse,importWarehousegoods,exportWarehousegoods,getGoods } from "@/api/goods/warehousegoods";
  export default {
    name: 'Warehousegoods',
    data() {
      return {
    ...
    methods: {
      ...
      loadData() {
        this.loading = true
        getWarehousegoodsList(this.searchData).then(res => {
          if(res){
            this.loading = false
      ...
      addForm() {
        this.$refs['formData'].validate((valid) => {
          if (valid) {
            var _this = this
            if (_this.editStatus) {
              editWarehousegoods(_this.formData).then(res => {
                if(res){
              ...
              addWarehousegoods(_this.formData).then(res => {
                if(res){
                  _this.formIsshow = false
                  _this.$modal.notifySuccess(res.errMsg)
                  _this.loadData()
      ...
      editStaff(row) {
        ...
        this.logicwarehouses = []
        const data1 = {warehouseId: this.formData.warehouseId, accessToken: sessionStorage.getItem('xn_token')}
        getLogicwarehouse(data1).then(res => {
          if(res){
      ...
      editDisable(scope) {
        ...
        disableWarehousegoods(data).then(res => {
          if(res){
      ...
      uploadSuccess(file){
        ...
        importWarehousegoods(param).then(res => {
          this.loading = false
          if(res){
            this.$modal.notifySuccess(res.errMsg)
      ...
      downMyOrder() {
        this.loading = true
        exportWarehousegoods(this.searchData).then(res => {
          this.loading = false
          if(res){
            this.$modal.notifySuccess(res.errMsg)
            if (res.msgData.downloadUrl !== undefined && res.msgData.downloadUrl !== '') {
      ...
      getLogicwarehouse(){
        ...
        this.logicwarehouses = []
        const data = {warehouseId: this.formData.warehouseId, accessToken: sessionStorage.getItem('xn_token')}
        getLogicwarehouse(data).then(res => {
          if(res){
            this.logicwarehouses = res.msgData.logicwarehouses
          }
        })
