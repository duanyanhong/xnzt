    ...
    <el-row :gutter="15">
      <el-col :span="21">
          ...
          <div class="head-container">
            <div>
              ...
              <template v-for="item in searchSetList">
                ...
                <!--销售出库单号(系统生成)-->
                <div v-if="item.englishName=='shCode'" class="filter-item">
                  <el-input
                    v-model="searchData.shCode"
                    size="small"
                    placeholder="输入销售出库单号"
                    style="width: 180px;"
                    clearable
                  >
                    <i  class="el-icon-edit el-input__icon"  slot="suffix"  @click="searchShow('shCode','输入销售出库单号')"></i>
        ...
        <el-row :gutter="15" style="margin-bottom: 1px;" class="crud-opts">
          <!--用户数据-->
          <el-col :span="20">
            <span class="crud-opts-left" style="padding-right: 10px;" v-hasPermi="['Order:DeliveryOrder:Examine']">
              <el-button class="filter-item" size="mini" type="danger"  icon="el-icon-finished" plain>审核</el-button>
            </span>
            <span class="crud-opts-left" style="padding-right: 10px;" v-hasPermi="['Order:DeliveryOrder:Decrypto']">
              <el-button class="filter-item" size="mini" type="success"  icon="el-icon-view" plain>订单解密</el-button>
            </span>
            <span class="crud-opts-left" style="padding-right: 10px;" v-hasPermi="['Order:DeliveryOrder:Reservation']">
              <el-button class="filter-item" size="mini" type="primary"  icon="el-icon-s-promotion" plain>预约马甲</el-button>
            </span>
            <span class="crud-opts-left" style="padding-right: 10px;" v-hasPermi="['Order:DeliveryOrder:Delete']">
              <el-button class="filter-item" size="mini" type="warning"  icon="el-icon-delete" plain>删除</el-button>
            </span>
            <span class="crud-opts-left" style="padding-right: 10px;" v-hasPermi="['Order:DeliveryOrder:Export']">
              <el-button class="filter-item" size="mini" type="warning"  icon="el-icon-download" plain @click="exportStockoutorder">导出订单</el-button>
            </span>
            <span class="crud-opts-left" style="padding-right: 10px;" v-hasPermi="['Order:DeliveryOrder:Exportdetail']">
              <el-button class="filter-item" size="mini" type="warning"  icon="el-icon-download" plain @click="exportStockoutorderDetail">导出明细</el-button>
            </span>
          </el-col>
          <el-col :span="2">
            <span >
...
<script>
  import { getStokoutorderList,setViewfield,setSearchfield,getShop,getLogicwarehouse,initShop,initLogicwarehouse,initPca,getCity,getArea,getOrderInfo,exportStockoutorder,exportStockoutorderDetail } from "@/api/order/stockoutorder";
  import Combined from "../deliveryorder/combined";
  import SearchSelect from "../deliveryorder/search";
  import GoodsDetail from "../deliveryorder/goodsDetail";
  import OrderInfo from "../deliveryorder/orderInfo";
  import AddressDetail from "../deliveryorder/addressDetail";
  import RemarkDetail from "../deliveryorder/remarkDetail";
  import TotalDetail from "../deliveryorder/totalDetail";
  import OperateLogs from "../deliveryorder/operateLogs";
  import ProcessTracing from "../deliveryorder/processTracing";
  export default {
    name: 'Stokoutorder',
    ...
    data() {
      return{
        ...
        titlesContent: '',
        searchType: '',
        searchData: {soCode: '', shCode: '', ownerId: [], shopId: [], deliveryOrderCode: '', preDeliveryOrderCode: '', preDeliveryOrderId: '', orderType: [], warehouseId: [], logicwarehouseId: [], sourcePlatformCode: [], createTime: [], placeOrderTime: [], payTime: [], sellerNick: '', buyerNick: '', vestNo: '', logisticsCode: [], logisticsName: [], expressCode: '', expressName: [], provinceId: [], cityId: [], areaId: [], orderState: [], toOutTime: [], page: 1, acceptTime: [], accessToken:sessionStorage.getItem('xn_token')},
    ...
    methods: {
      loadData() {
        ...
        getStokoutorderList(data).then(res => {
          if(res){
            this.loading = false
      ...
      addShowDetail(row){
        ...
        getOrderInfo({stockoutorderId:row.id,accessToken: sessionStorage.getItem('xn_token')}).then(res => {
          if(res){
      ...
      exportStockoutorder() {
        ...
        data.groupId = this.groupId
        data.acceptTime = this.searchData.acceptTime
        exportStockoutorder(data).then(res => {
          this.loading = false
          if(res){
            this.$modal.notifySuccess(res.errMsg)
      ...
      exportStockoutorderDetail() {
        ...
        data.groupId = this.groupId
        data.acceptTime = this.searchData.acceptTime
        exportStockoutorderDetail(data).then(res => {
          this.loading = false
          if(res){
            this.$modal.notifySuccess(res.errMsg)
            if (res.msgData.downloadUrl !== undefined && res.msgData.downloadUrl !== '') {
