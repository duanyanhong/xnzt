<template>
  <div class="app-container">
        ...
        <div class="crud-opts">
          <span class="crud-opts-left">
            ...
            <el-button
              ...
              @click="addShow"
              plain
              v-hasPermi="['Stock:Allocation:Add']"
            >
              新增
            </el-button>
            <el-button
              ...
              plain
              v-hasPermi="['Stock:Allocation:Examine']"
            >
              审核
            </el-button>
            <el-button
              ...
              plain
              v-hasPermi="['Stock:Allocation:Remove']"
            >
              删除
            </el-button>
            <el-button
              ...
              @click="interceptAllocation"
              plain
              v-hasPermi="['Stock:Allocation:Intercept']"
            >
              拦截
            </el-button>
          </span>
          <span class="crud-opts-left">
              <el-upload
                ...
                accept=".xlsx,.xls,.csv"
                action="#"
                v-hasPermi="['Stock:Allocation:Import']"
              >
          ...
          <span class="crud-opts-left">
            <el-button
              ...
              plain
              @click="downMyOrder"
              v-hasPermi="['Stock:Allocation:Export']"
            >导出</el-button>
          </span>
        </div>
        <u-table
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-button v-hasPermi="['Stock:Allocation:Preview']" size="mini" type="text" style="width: 50px;" icon="el-icon-edit" @click="editStaff(scope.row)"  >编辑明细</el-button>
            </template>
          </u-table-column>
        </u-table>
        <!--分页组件-->
    ...
    <el-dialog append-to-body :close-on-click-modal="false" top="8vh" :visible.sync="formDetailIsshow"  title="调拨单明细管理" width="80%">
      <OrderDetail :detailData="detailData" :detailInfo.sync="detailInfo"></OrderDetail>
      <div slot="footer" class="dialog-footer" style="text-align: center;">
        <el-button type="primary" v-hasPermi="['Stock:Allocation:Save']" v-debounce="addDetailForm" >保存</el-button>
        <el-button type="primary" @click="formDetailIsshow=false">取消</el-button>
      </div>
    </el-dialog>
    <el-dialog append-to-body :close-on-click-modal="false" top="8vh" :visible.sync="formIsshow"  title="调拨单明细管理" width="80%">
      <AddOrderDetail :logicwarehouses="logicwarehouses" :owners="owners" :formIsshow.sync="formIsshow" :addDetailInfo.sync="addDetailInfo" :loadData="loadData"></AddOrderDetail>
    </el-dialog>
  </div>
</template>

<script>
  import { getAllocationList,getAllocationDetail,addAllocationDetail,importAllocation,exportAllocation,examineAllocation,removeAllocation,interceptAllocation } from "@/api/stock/allocationorder";
  import OrderDetail from "./orderDetail";
  import AddOrderDetail from "./addOrderDetail";
  export default {
    name: 'Allocation',
    components: { OrderDetail,AddOrderDetail },
    data() {
      return {
        searchData: {
          limit: 20,
    ...
    methods: {
      ...
      loadData() {
        this.loading = true
        getAllocationList(this.searchData).then(res => {
          if(res){
            this.loading = false
      ...
      downMyOrder() {
        this.loading = true
        exportAllocation(this.searchData).then(res => {
          this.loading = false
          if(res){
      ...
      addDetailForm(){
        ...
        var data = {id:this.detailData.id,details:this.detailInfo,accessToken: sessionStorage.getItem('xn_token')}
        addAllocationDetail(data).then(res => {
          if(res){
            this.$modal.notifySuccess(res.errMsg)
            this.formDetailIsshow = false
          }
        })
      },
      uploadSuccess(file){
        ...
        importAllocation(param).then(res => {
          this.loading = false
          if(res){
            this.$modal.notifySuccess(res.errMsg)
            if (res.msgData.downloadUrl !== undefined && res.msgData.downloadUrl !== '') {
      ...
      examineAllocation() {
        ...
        _this.$confirm('确定要操作选中数据吗?', '提示', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'}).then(() => {
          examineAllocation({ids:this.ids,accessToken: sessionStorage.getItem('xn_token')}).then(res => {
            if(res){
              this.$modal.notifySuccess(res.errMsg)
              this.loadData()
            }
      ...
      removeAllocation() {
        ...
        _this.$confirm('确定要操作选中数据吗?', '提示', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'}).then(() => {
          removeAllocation({ids:this.ids,accessToken: sessionStorage.getItem('xn_token')}).then(res => {
            if(res){
              this.$modal.notifySuccess(res.errMsg)
              this.loadData()
            }
      ...
      interceptAllocation() {
        ...
        _this.$confirm('确定要操作选中数据吗?', '提示', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'}).then(() => {
          interceptAllocation({ids:this.ids,accessToken: sessionStorage.getItem('xn_token')}).then(res => {
            if(res){
              this.$modal.notifySuccess(res.errMsg)
              this.loadData()
            }
